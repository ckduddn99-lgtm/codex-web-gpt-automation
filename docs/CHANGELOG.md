...
- Control Plane 생존성을 별도 `control_plane_guardian.py` + systemd guardian/timer로 분리했습니다. guardian은 Project Control/Commander/Tailscale 상태와 memory/io PSI·load·swap pressure를 감시하고, allowlisted control service만 복구하며, 심각한 압력에서는 기존 fail-closed cleanup을 cooldown 기반으로 호출합니다. Project Control HTTP/watchdog에는 CPU/IO/OOM/memory 우선순위를 부여하고 watchdog의 `Requires=project-control-http.service` 결합을 제거해 adapter 장애와 독립적으로 복구가 가능하도록 했습니다. Discord 5분 heartbeat도 별도 service/timer 템플릿으로 분리했으며, 설치 전 호환성을 위해 기존 watchdog 호출은 유지합니다. focused verification: resilience suites `39 passed`.
- 실제 장애에서 1GB host의 Chrome/worker가 swap/page I/O를 밀어붙여 I/O PSI full avg10 약 96%, memory PSI full avg10 약 51%, iowait 97~99%, 다수 D-state를 만들며 Project Control까지 stall시키는 것이 확인됐습니다. Project Control을 최고 CPU/IO weight와 memory low/min로 보호하고 Oracle Chrome/ChatGPT browser worker를 낮은 CPU/IO weight, 높은 Nice/OOMScoreAdjust, MemoryHigh/Max로 격리했습니다. watchdog은 restart 권한을 버리고 observer-only로 바꿔 guardian과의 재시작 경합을 제거했고, guardian은 emergency pressure에서 browser worker와 Chrome을 먼저 희생합니다. `chatgpt-browser-demand.timer`를 추가해 pending ChatGPT bus task가 있고 host pressure가 낮을 때만 Chrome을 기동하고, worker는 oneshot 한 건만 처리한 뒤 idle 시 Chrome을 다시 내립니다. host health는 PSI와 D-state 목록을 직접 보고합니다. focused verification: browser-demand/systemd/guardian/watchdog `19 passed`; 실제 설치 후 Project Control endpoint와 4개 timers healthy/active 확인.
- 외부 break-glass를 상시 VM에 의존하지 않도록 `Telegram -> Cloudflare Worker -> GitHub Actions -> agent-box SSH` 설계를 추가했습니다. Worker는 Telegram secret header와 정확한 chat/user ID를 모두 확인하고 `/heal`, `/status`, `/restart_pc`, `/stop_browser` 네 명령만 `repository_dispatch`로 전달합니다. Actions workflow도 같은 네 명령만 허용하며 strict known-hosts와 전용 SSH identity를 요구합니다. agent-box 쪽 forced-command helper는 `SSH_ORIGINAL_COMMAND`를 이 네 명령으로 제한하고 `project-control-ops`의 기존 allowlist만 호출합니다. focused verification: relay/forced-command tests `6 passed`. 계정 비밀값과 실제 Cloudflare/GitHub secret 배포는 저장소에 기록하지 않습니다.
