...
- 서버 ChatGPT bus/goal worker는 provider lock으로 직렬화되는 관리형 실행이므로 매 run마다 `--copy-profile`로 새 동적 CDP 브라우저를 만들지 않고, systemd가 관리하는 signed-in Chrome `127.0.0.1:9222`에 `--remote-chrome 127.0.0.1:9222`로 직접 연결하도록 변경했습니다. `--browser-attach-running`은 Oracle의 별도 attach-metadata 계약을 요구해 관리형 systemd Chrome을 거절하므로 서버 경로에서는 사용하지 않습니다. `chatgpt-server-worker@` unit 템플릿도 `oracle-browser@`를 Requires/After/Wants에 포함합니다.
