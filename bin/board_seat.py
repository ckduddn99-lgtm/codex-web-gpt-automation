...
- 파일 수정, 커밋, 배포, 서비스 재기동. 이 좌석은 **읽고 확인하고 말하는** 자리다.
  ...
  방에 그렇게 쓰고 넘겨라.
- **자격증명 파일을 열지 마라.** `.env`, `*.key`, `*.pem`, `credentials*`, `*token*`,
  `*secret*`. 이 트리에는 실제 매매 계좌 자격증명이 들어 있는 파일이 있고, 한 번 읽으면
  당신의 컨텍스트를 통해 모델 제공자에게 나간다. 설정값을 알아야 하면 그 값을 읽지 말고
  **어느 파일의 어느 키인지**만 방에 적어라 — 다른 좌석이 확인해 준다.
